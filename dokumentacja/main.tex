...
Za pomocą menadżera pakietów NPM uruchamiane są poszczególne moduły systemu. Aby uruchomić dany moduł należy otworzyć katalog z tym modułem, w tym katalogu znajduje się plik \textit{package.json}, który zawiera konfigurację i skrypty uruchamiające program. Programy uruchamia się za pomocą poleceń dostarczanych przez pakiet NPM, które należy wpisać w konsoli. Należy uruchomić konsolę w folderze z plikiem \textit{package.json}.
...
Aplikacja serwerowa \textit{e-voting-api} domyślnie uruchamiana jest na porcie 8080, jednak można to zmienić za pomocą następującego polecenia:
